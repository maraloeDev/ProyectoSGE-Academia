page 50118 "Course with the lowest fee"
{
    PageType = List;
    ApplicationArea = All;
    UsageCategory = Lists;
    SourceTable = "Secretary/Admon. Table";
    Caption = 'Course with the lowest fee', comment = 'ESP="Curso con la tarifa mas baja"';
    ...
    layout
    {
        area(content)
        {
            repeater(General)
            {
                ...
                field("COURSE FEE"; Rec."Lowest Fee Course")
                {
    ...
    actions
    {
        ...
        area(Navigation)
        {
            action("Total estudiantes")
            {
                ...
                trigger OnAction()
                var
                    CourseRecord: Record Course;
                begin
                    CourseRecord.SETFILTER("Course Fee", '<=%1', Rec."Lowest Fee Course");
                    estudiantes.GetTotalEnrolledStudents(CourseRecord);
                end;
            }
